export class Vertex {
  constructor(vid, x, y, z, xn, yn, zn, color = [1.0, 1.0, 1.0, 1.0]) {
    this.vid = vid;

    this.position = [x, y, z, 1];
    this.normal = [xn, yn, zn, 0];
...
export class HalfEdgeDS {
  ...
  build(coords, norm,  trigs, v) {
    ...
    for (let vid = 0; vid < coords.length; vid+=3) {
      ...
      const v = new Vertex(vid / 3, x, y, z, xn, yn, zn);
    ...
    for (let tid = 0; tid < trigs.length; tid+=3) {
      ...
      const v2  = this.vertices[ trigs[tid + 2] - 1];
  ...
  estrela(v) {
    ...
    //this.vertices[v].color = [1.0, 0.0, 0.0, 0.0];

    var initial = this.vertices[v].he;
    initial.vertex.color = [1.0, 0.0, 0.0, 0.0];
    initial.next.vertex.color = [1.0, 0.0, 0.0, 0.0];
    initial.next.next.vertex.color = [1.0, 0.0, 0.0, 0.0];
    if(initial.opposite.next != null){
      var edge = initial.opposite.next;
      while(edge != initial){
        edge.next.vertex.color = [1.0, 0.0, 0.0, 0.0];
        edge.next.next.vertex.color = [1.0, 0.0, 0.0, 0.0];
